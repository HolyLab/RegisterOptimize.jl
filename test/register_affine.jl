...
using Images, CoordinateTransformations, Rotations, TestImages, LinearAlgebra
using RegisterCore, RegisterPenalty, RegisterDeformation, RegisterMismatch, RegisterFit
using RegisterOptimize

@testset "Derivative-based rigid registration" begin
    #### Rigid registration
    fixed = sin.(range(0, stop=pi, length=101)).*range(5, stop=7, length=97)'
    ...
    S = tfprod.linear
    @test abs(S[1,2]) < 0.05
    translation = tfprod.translation
    @test all(abs.(translation) .< 0.03)
end
...
#NOTE: the 2D test below fails rarely and the 3D test fails often, apparently because full affine is too difficult with these images
    ...
    mat = SArray{Tuple{2,2}}(Matrix{Float64}(I,2,2) + rand(2,2)./40 + -rand(2,2)./40)
    ...
    ##random displacement from the identity matrix
    #mat = SArray{Tuple{3,3}}(Matrix{Float64}(I,3,3) + rand(3,3)./30 + -rand(3,3)./30);
    #tfm0 = AffineMap(mat, shft); #ground truth
    #newfixed = warp(moving, tfm0);
    ...
    ##random displacement from the identity matrix
    #mat = SArray{Tuple{3,3}}(Matrix{Float64}(I,3,3) + rand(3,3)./50 + -rand(3,3)./50);
...
to_affine(tfm::Translation) = (m = length(tfm.translation); AffineMap(Matrix{Float64}(I,m,m), tfm.translation))
to_affine(tfm::LinearMap) = AffineMap(tfm.linear, zeros(size(tfm.linear,1)))
...
function tfmtest(tfm, tfminv)
    ...
    @test all(x->(-offdiagtol < x < offdiagtol), comp.linear.-diagm(0=>diag(comp.linear)))
