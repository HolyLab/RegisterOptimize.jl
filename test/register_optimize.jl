...
function initial_guess_direct(A, cs::Matrix, Qs::Matrix)
    ...
    x = Ac \ b
    return reshape(reinterpret(SVector{2, Float64}, vec(x)), size(Qs))
end
...
@testset "Register Optimize" begin
    ...
    for I in eachindex(u)
        @test ≈(u[I], cs[I], atol = 1.0e-3)
        @test ≈(ux[I], cs[I], atol = 1.0e-3)
    end
    ...
    Qs0 = [reshape([1], 1, 1) for i in 1:3]
...
@testset "Optimization with a temporal penalty" begin
    ...
    ϕs = tighten([GridDeformation(u[:, :, :, t], nodes) for t in 1:3])
    g = similar(u)
    ...
    for (u1, val) in (
        ...
        for uv in u1
            @test ≈(uv, val, atol = 1.0e-2)  # look into why this is so low
        end
    end
end
